...
// const { unwatchFile } = require('jest-plugin-fs/mock')
...
describe('instance methods', () => {
  ...
  beforeEach(() => {
    utils.createKeyValueObjectFromFlag.mockRestore()
    utils.createKeyValueObjectFromFile.mockRestore()
    utils.deployPackage.mockRestore()
    utils.processPackage.mockRestore()
    utils.setPaths.mockRestore()
  ...
  describe('run', () => {
    ...
    test('run calls through to runtime-lib.utils', async () => {
      utils.setPaths.mockImplementation(() => ({
        packages: {},
        deploymentTriggers: [],
        deploymentPackages: {},
      ...
      expect(utils.createKeyValueObjectFromFlag).not.toHaveBeenCalled()
      expect(utils.createKeyValueObjectFromFile).not.toHaveBeenCalled()

      expect(utils.processPackage).toHaveBeenCalled() // TODO: calledWith( ... )
      expect(utils.deployPackage).toHaveBeenCalled()
    ...
    test('run calls createKeyValueObjectFromFlag with --param', async () => {
      utils.setPaths.mockImplementation(() => ({
        packages: {},
        deploymentTriggers: [],
        deploymentPackages: {},
      ...
      await command.run()
      expect(utils.setPaths).toHaveBeenCalledWith(expect.objectContaining({ param: expect.any(Array), useragent: expect.any(String) }))
      expect(utils.createKeyValueObjectFromFlag).toHaveBeenCalledWith(['key', 'value'])
      expect(utils.createKeyValueObjectFromFile).not.toHaveBeenCalled()
    ...
    test('run calls createKeyValueObjectFromFlag with multiple --param', async () => {
      utils.setPaths.mockImplementation(() => ({
        packages: {},
        deploymentTriggers: [],
        deploymentPackages: {},
      ...
      await command.run()
      expect(utils.setPaths).toHaveBeenCalledWith(expect.objectContaining({ param: expect.any(Array), useragent: expect.any(String) }))
      expect(utils.createKeyValueObjectFromFlag).toHaveBeenCalledWith(['key', 'value', 'key2', 'value2'])
      expect(utils.createKeyValueObjectFromFile).not.toHaveBeenCalled()
    ...
    test('run calls createKeyValueObjectFromFile with -P', async () => {
      utils.setPaths.mockImplementation(() => ({
        packages: { p1: 'v1' },
        deploymentTriggers: [],
        deploymentPackages: {},
      ...
      utils.processPackage.mockImplementation(() => {
        return [{ something: 'goes here' }]
      })
      aioConfig.get.mockImplementation(() => {
        return 'fake-config-ims-org-id'
      })
      command.argv = ['-P', 'param-file']
      await command.run()
      expect(utils.setPaths).toHaveBeenCalledWith(expect.objectContaining({ 'param-file': 'param-file', useragent: expect.any(String) }))
      expect(utils.createKeyValueObjectFromFlag).not.toHaveBeenCalled()
      expect(utils.createKeyValueObjectFromFile).toHaveBeenCalledWith('param-file')
    ...
    test('run throws if utils.deployPackage throws', async () => {
      ...
      command.argv = ['-P', 'param-file']
      await expect(command.run()).rejects.toEqual(expect.any(Error))
