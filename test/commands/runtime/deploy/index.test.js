      ...
      test('deploys web action with require-adobe-auth annotation', () => {
        ...
        return command.run()
          .then(() => {
            ...
            // generated sequences
            expect(cmd).toHaveBeenCalledWith({ name: 'testSeq/helloAction', action: '', annotations: { 'web-export': true }, exec: { components: [VALIDATOR, '/ns/testSeq/__secured_helloAction'], kind: 'sequence' } })
            expect(cmd).toHaveBeenCalledWith({ name: 'testSeq/helloAction2', action: '', annotations: { 'web-export': true }, exec: { components: [VALIDATOR, '/ns/testSeq/__secured_helloAction2'], kind: 'sequence' } })
            // pkg2
            // action
            ...
            // sequence
            expect(cmd).toHaveBeenCalledWith({ name: 'demo_package/sampleAction', action: '', annotations: { 'web-export': true, 'raw-http': true }, exec: { components: [VALIDATOR, '/ns/demo_package/__secured_sampleAction'], kind: 'sequence' } })
